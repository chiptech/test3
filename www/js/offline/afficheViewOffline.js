...
sessionStorage.setItem("EntityName",sessionStorage.getItem('offlineDB'));
	sessionStorage.setItem("guidEntity",guid[j]);

	$records = $.parseJSON(sessionStorage.getItem("offLineRecords"));
	var entityRecord = JSON.stringify($records.Entities[j]);
	...
	window.location= sessionStorage.getItem('offlineDB')+'FormOffline.html' ;
}
function goback()
{
